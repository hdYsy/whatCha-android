...
//    private boolean mFullscreenEnabled = false;
    ...
    private Object progressPosition;
...
//        String s = mCurrentTime.getText().toString();
        return   mPlayer.getCurrentPosition() / 1000;
    }
    //跳到当前时间
    public void setProgressPosition(int positioin){
       mPlayer.seekTo( positioin*1000);
    }
    //获取这个 progress
...
//        String s = mCurrentTime.getText().toString();
     ...
     * @param progress
     */
    public void setProgressBar1000L(int progress){
        int position = mPlayer.getCurrentPosition();
        int duration = mPlayer.getDuration();
        if (mProgress != null) {
            if (duration > 0) {
                // use long to avoid overflow
                long pos = 1000L * progress / duration;
                mProgress.setProgress((int) pos);
            }
        }
    }
